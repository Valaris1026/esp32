...
void setup(void) {
  ...
  server.begin();
  IPAddress TCP_host = WiFi.gatewayIP();                            // get gate way ip
  vTaskDelay(100);
  String GateWay = TCP_host.toString();
  GateWay.toCharArray(Host, 16, 0);
  debug("GateWay:%s", Host);
...
void loop(void) {
  ...
  {
    String tcpRcv=myClient.readString();
